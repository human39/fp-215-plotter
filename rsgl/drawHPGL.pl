...
# Modified to produce RSGL (TRS-80 FP-215) output by Matt Boytim, maboytim@yahoo.com
# 
# Program draws HPGL-files to screen using Gtk-Perl.
...
$window->set_title( "Draw from HPGL" );
...
$button = new Gtk::Button( "Paint HPGL picture" );
...
my $label = new Gtk::Label("Scale % (1-100)");
$hbox->pack_start( $label, $false, $false, 0);
$label->show();

my $adj = new Gtk::Adjustment( 25.0, 1.0, 100.0, 0.5, 4.0, 0.0 );
	...
	while (<FILE>)
	{
		...
		while ($row =~ m/;/g)
		{
			...
			if ($cmd ne "")
			{
				...
				elsif (substr($cmd,0,2) eq "PU") {$mode = "M";}
